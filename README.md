...
**Q：打开登录弹窗后，页面已显示登录态，但插件一直「等待扫码」、不自动关闭？**

A：这是 Obsidian 仍加载着旧的 `main.js` 所致。请**彻底重启 Obsidian**（或在「设置 → 社区插件」里先关闭再打开本插件）以重载代码；若重启后仍无效，请在设置页点击「注销」后重新扫码，原 Cookie 可能已过期。

**Q：注销后重新登录没反应？**

A：同样先按上述方式重载插件。务必使用插件内的「注销」按钮退出，而不是直接清空浏览器 Cookie——插件的登录态存储在 Electron 会话中，需由插件自身清理才能彻底生效。

**Q：扫码登录按钮点了没反应 / 弹不出窗口？**

A：本插件基于 Electron 的 `BrowserWindow`，仅支持 **Obsidian 桌面端**（移动端 / 网页端不可用）。请确认使用的是桌面版；若仍异常，打开开发者控制台（`Ctrl + Shift + I`）查看 `[zhihu-loader]` 开头的报错日志。
